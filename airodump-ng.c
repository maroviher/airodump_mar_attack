...
void trim(char *str)
{
	...
	while (isspace((int )str[begin]))
		begin++;
	while ((end >= begin) && isspace((int )str[end]))
		end--;
	...
	if (G.show_ap)
	{
		...
		if (G.show_wps)
		{
			...
			if (ws_col > (columns_ap - 4))
			{
				...
				snprintf(strbuf + columns_ap + G.maxsize_wps_seen - 6, 9, "%s",
						"   ESSID");
				if (G.show_manufacturer)
				{
					memset(strbuf + columns_ap + G.maxsize_wps_seen + 2, 32,
							G.maxsize_essid_seen - 5);
					snprintf(strbuf + columns_ap + G.maxsize_essid_seen - 5, 15,
							"%s", "  MANUFACTURER");
				}
			}
		}
			...
			if (ws_col > (columns_ap - 4))
			{
				...
				if (G.show_wps)
				{
					if (ap_cur->wps.state != 0xFF)
					{
						...
						{
							snprintf(strbuf, sizeof(strbuf) - 1, "%d.%d",
				...
				if (ap_cur->essid[0] != 0x00)
				{
					if (G.show_wps)
						snprintf(strbuf + G.maxsize_wps_seen,
								sizeof(strbuf) - G.maxsize_wps_seen, "  %s",
								ap_cur->essid);
					else
				...
				else
				{
					if (G.show_wps)
						snprintf(strbuf + G.maxsize_wps_seen,
								...
								"  <length:%3d>%s", ap_cur->ssid_length,
								"\x00");
					else
...
int dump_write_csv(void)
{
	...
	while (st_cur != NULL)
	{
		...
		for (i = 0, n = 0; i < NB_PRB; i++)
		{
			...
			temp = format_text_for_csv(st_cur->probes[i],
...
int dump_write_kismet_netxml_client_info(struct ST_info *client, int client_no)
{
	...
	fprintf(G.f_kis_xml, "\t\t<wireless-client number=\"%d\" "
			"type=\"%s\" first-time=\"%s\""
			" last-time=\"%s\">\n", client_no,
			(is_unassociated) ? "tods" : "established", first_time, last_time);

	fprintf(G.f_kis_xml,
			"\t\t\t<client-mac>%02X:%02X:%02X:%02X:%02X:%02X</client-mac>\n",
			client->stmac[0], client->stmac[1], client->stmac[2],
			client->stmac[3], client->stmac[4], client->stmac[5]);
	...
	client_max_rate =
			(client->rate_from > client->rate_to) ?
					client->rate_from : client->rate_to;
...
int dump_write_kismet_netxml(void)
{
	...
	while (ap_cur != NULL)
	{
		...
		{
			if (ap_cur->security & ENC_TKIP)
				fprintf(G.f_kis_xml, NETXML_ENCRYPTION_TAG, "\t\t\t",
						"WPA+TKIP");
			if (ap_cur->security & AUTH_MGT)
				fprintf(G.f_kis_xml, NETXML_ENCRYPTION_TAG, "\t\t\t",
		...
		fprintf(G.f_kis_xml, "\t\t<freqmhz>%d %ld</freqmhz>\n",
				(ap_cur->channel) == -1 ?
						0 : getFrequencyFromChannel(ap_cur->channel),
		...
		fprintf(G.f_kis_xml, "\t\t<snr-info>\n"
				...
				"\t\t\t<max_noise_rssi>0</max_noise_rssi>\n"
				"\t\t</snr-info>\n", average_power, average_power,
				average_power, max_power, max_power);
	...
	while (st_cur != NULL)
	{
				...
				|| memcmp(st_cur->base->bssid, BROADCAST, 6) == 0)
		{
			...
			/* Freq (in Mhz) and total number of packet on that frequency
			 FIXME: Take G.freqoption in account */
			fprintf(G.f_kis_xml, "\t\t<freqmhz>%d %ld</freqmhz>\n",
					getFrequencyFromChannel(st_cur->channel), st_cur->nb_pkt);
 ...
 */
static int json_get_value_for_name(const char *buffer, const char *name,
		char *value)
{
	char * to_find;
	char *cursor;
	...
	if (buffer == NULL
			|| strlen(buffer)
					== 0|| name == NULL || strlen(name) == 0 || value == NULL)
	{
		return 0;
	}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		option = getopt_long(argc, argv,
				"p:b:c:egiw:s:t:u:m:d:N:R:aHDBK:Ahf:r:EC:o:x:MUI:W", long_options,
				&option_index);
		...
		case 'I':
			...
			{
				printf(
						"Error: Write interval is not a number (>0). Aborting.\n");
				exit(1);
			}
			...
			{
				printf(
						"Error: Write interval must be greater than 0. Aborting.\n");
				exit(1);
			}
			break;
